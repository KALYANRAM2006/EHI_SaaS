          ...
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 sm:gap-8 min-w-0">
              <div className="flex items-center gap-2 cursor-pointer shrink-0" onClick={() => navigate('/')}>
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-xl overflow-hidden shadow-lg" style={{boxShadow:'0 4px 14px rgba(59,130,246,0.3)'}}>
                  <img src="/clinquilt.svg" alt="ClinQuilt" className="w-8 h-8 sm:w-10 sm:h-10" />
                </div>
                <div>
                  <span className="text-lg sm:text-2xl font-bold bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">ClinQuilt</span>
            ...
            <div className="flex items-center gap-1 sm:gap-2 shrink-0">
              ...
              <button
                ...
                className="inline-flex items-center gap-1 px-2 py-1.5 sm:px-3 bg-indigo-50 border border-indigo-200 rounded-full text-xs font-medium text-indigo-700 hover:bg-indigo-100 transition-colors group"
                title="AI Settings"
              >
                <Sparkles className="w-3.5 h-3.5 text-indigo-500 group-hover:scale-110 transition-transform" />
                <span className="hidden sm:inline">AI: {aiConfig.mode === 'local' ? 'Local' : aiConfig.mode === 'cloud' ? 'Cloud' : 'Browser'}</span>
              </button>
              {parsedData?.patients?.length > 1 && (
                <select
                  value={selectedPatient?.patId || ''}
                  onChange={(e) => selectPatient(e.target.value)}
                  className="text-xs sm:text-sm border border-gray-300 rounded-lg px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-500 max-w-[80px] sm:max-w-none"
                >
              ...
              <button
                ...
                className="p-1.5 sm:p-2 text-red-500 hover:bg-red-50 rounded-xl transition-colors group relative"
                title="Clear all data & return home"
              >
                <Trash2 className="w-4 h-4 sm:w-5 sm:h-5 group-hover:scale-110 transition-transform" />
              </button>
              <button className="hidden sm:block p-2 text-gray-600 hover:bg-gray-100 rounded-xl transition-colors">
                <HelpCircle className="w-5 h-5" />
              </button>
              <button className="p-1.5 sm:p-2 text-gray-600 hover:bg-gray-100 rounded-xl transition-colors">
